...
In the last lesson, you learned the parts of an {{< glink expression >}}: its terms, its coefficients, and its {{< glink slug="variable" text="variables" >}}. An expression like $4x + 1$ tells you what to do with a number. But it has no value of its own until you know what $x$ is.

In this lesson, you’ll find that value. You replace the variable with a number, then simplify using the order of operations. That is what it means to {{< glink evaluate >}} an expression.

You’ll also use this to test whether two expressions are {{< glink slug="equivalent-expressions" text="equivalent" >}}.
